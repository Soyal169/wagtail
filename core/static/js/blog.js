...
function initBlogFilters() {
  ...
  filterButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
      // Filter buttons are real links to "?category=..." so filtering works
      // without JS (and is shareable/crawlable); when JS is available we
      // intercept the click, update the URL, and filter in place instead
      // of doing a full page reload.
      if (btn.tagName === 'A' && btn.href) {
        e.preventDefault();
        window.history.pushState({}, '', btn.getAttribute('href'));
      }
